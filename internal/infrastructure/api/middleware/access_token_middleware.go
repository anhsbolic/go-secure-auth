...
func AccessTokenMiddleware(db *sqlx.DB) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		...
		// check session with alias, sessionId, and accessUUID
		...
		query := `SELECT * FROM user_sessions WHERE alias = $1 AND session_id = $2 AND access_uuid = $3  
                              AND ip_address_hash = $4 AND user_agent_hash = $5 AND expires_at > NOW()  
                              AND revoked_at IS NULL AND deleted_at IS NULL`
		err = db.GetContext(ctx.Context(), &userSession, query, alias, sessionID, accessUUID, ipAddressHash, userAgentHash)
		if err != nil {
